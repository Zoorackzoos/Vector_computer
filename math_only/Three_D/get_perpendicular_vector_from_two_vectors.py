...
def get_perpendicular_vector_from_two_vectors(vector_a,vector_b,tab_amount):
    ...
    result_of_algebraic_dot_product = cross_product(vertex_a=vector_a, vertex_b=vector_b, tab_amount=tab_amount + '\t')
    print(f"{tab_amount}\t{result_of_algebraic_dot_product}")
    result_of_algebraic_dot_product_ABSified = ABS_for_vectors_AKA_distance_formula(
        vertex=result_of_algebraic_dot_product, tab_amount=tab_amount + '\t')
